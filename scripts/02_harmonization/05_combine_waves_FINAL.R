# ==============================================================================
# FINAL Wave Combination for 2002-2007 Survey Data
# Focus: Immigration Attitudes, Rich Demographics, and Policy Analysis
# ==============================================================================
...
validate_wave_data_FINAL <- function(data, year) {
  ...
  # Check key immigration attitude variables
  attitude_vars <- c("immigration_levels_opinion", "legalization_support", "deportation_worry")
  working_attitudes <- 0
  for (var in attitude_vars) {
    if (var %in% names(data)) {
      values <- data[[var]][!is.na(data[[var]])]
      if (length(unique(values)) >= 2) {
        working_attitudes <- working_attitudes + 1
      }
    }
  }
  
  if (working_attitudes == 0) {
    warnings <- c(warnings, "No working immigration attitude variables found")
  } else {
    cat("✓ Found", working_attitudes, "working immigration attitude variables\n")
  }
  
  # Check demographic variables
  demo_vars <- c("gender", "age", "education", "income", "marital_status", "employment", "state")
  working_demos <- 0
  for (var in demo_vars) {
    if (var %in% names(data)) {
      values <- data[[var]][!is.na(data[[var]])]
      if (length(unique(values)) >= 2) {
        working_demos <- working_demos + 1
      }
    }
  }
  
  cat("✓ Found", working_demos, "working demographic variables\n")
  ...
  if (length(warnings) > 5) {
    cat("WARNINGS (showing first 5):\n")
    for (warning in warnings[1:5]) {
      cat("- ", warning, "\n")
    }
    cat("... and", length(warnings) - 5, "more warnings\n")
...
create_variable_summary_FINAL <- function(data) {
  ...
  for (var in variables) {
    ...
    notes <- case_when(
      ...
      n_unique > 50 & var %in% c("citizenship_status", "place_birth", "immigrant_generation", "gender", "political_party", "marital_status", "employment", "education") ~ "WARNING: Too many unique values for categorical variable",
      pct_missing > 95 ~ "WARNING: Extremely high missingness",
      pct_missing > 80 ~ "WARNING: High missingness", 
      pct_missing < 10 & n_unique >= 2 ~ "EXCELLENT: Low missingness, good variation",
      pct_missing < 30 & n_unique >= 2 ~ "GOOD: Reasonable missingness, good variation",
      n_unique >= 2 ~ "OK: Adequate variation",
      TRUE ~ "OK"
    )
    
    # Add concept mapping info with immigration focus
    concept_mapping <- case_when(
      var %in% c("gender", "age") ~ "Demographics",
      ...
      var %in% c("immigration_levels_opinion", "legalization_support", "deportation_worry") ~ "Immigration Attitudes",
      var %in% c("political_party") ~ "Political",
      var %in% c("state") ~ "Geographic",
      TRUE ~ "Other"
    )
  ...
  summary_data <- do.call(rbind, summary_list) %>%
    arrange(concept, desc(n_unique), pct_missing)
...
combine_waves_FINAL <- function(cleaned_data_dir = "cleaned_data_FINAL") {
  ...
  for (file in cleaned_files) {
    ...
    # Validate the data
    is_valid <- validate_wave_data_FINAL(data, year)
  ...
  # Ensure consistent column ordering - prioritize immigration attitude variables
  col_order <- c("survey_year", 
                 # Immigration attitudes (KEY FOCUS!)
                 "immigration_levels_opinion", "legalization_support", "deportation_worry",
                 # Demographics
                 "gender", "age", "education", "income", "marital_status", "employment", "state",
                 # Immigration/Nativity
                 "citizenship_status", "place_birth", "parent_nativity", "immigrant_generation",
                 # Political
                 "political_party")
  ...
  var_summary <- create_variable_summary_FINAL(combined_data)
  print(var_summary)
  ...
  concept_summary <- var_summary %>%
    ...
    summarise(
      ...
      n_excellent = sum(str_detect(notes, "EXCELLENT")),
      n_good = sum(str_detect(notes, "GOOD")),
      n_ok = sum(str_detect(notes, "OK")),
      n_warnings = sum(str_detect(notes, "WARNING")),
      n_critical = sum(str_detect(notes, "CRITICAL")),
      .groups = "drop"
    ) %>%
    arrange(desc(n_excellent), desc(n_good), avg_missing)
  
  print(concept_summary)
  
  # Immigration attitude analysis
  cat("\n=== Immigration Attitude Variables Analysis ===\n")
  attitude_vars <- c("immigration_levels_opinion", "legalization_support", "deportation_worry")
  
  for (var in attitude_vars) {
    if (var %in% names(combined_data)) {
      values <- combined_data[[var]][!is.na(combined_data[[var]])]
      if (length(values) > 0) {
        cat(sprintf("%-25s: %d unique values, %d observations across %d years\n", 
                   var, length(unique(values)), length(values), 
                   length(unique(combined_data$survey_year[!is.na(combined_data[[var]])]))))
        
        # Show distribution by year
        year_dist <- combined_data %>%
          filter(!is.na(.data[[var]])) %>%
          group_by(survey_year) %>%
          summarise(n = n(), .groups = "drop")
        
        if (nrow(year_dist) > 0) {
          cat("  Year distribution:", paste(year_dist$survey_year, collapse = ", "), "\n")
        }
      }
    }
  }
  ...
  # Check immigration attitude coverage
  working_attitudes <- var_summary %>%
    filter(concept == "Immigration Attitudes" & n_unique >= 2) %>%
    nrow()
  
  if (working_attitudes < 2) {
    quality_issues <- c(quality_issues, paste("Only", working_attitudes, "working immigration attitude variables"))
  } else {
    cat("✓ ", working_attitudes, "working immigration attitude variables for temporal analysis\n")
  }
...
main_combine_waves_FINAL <- function() {
  cat("=== FINAL Wave Combination Process for Immigration Attitude Analysis ===\n\n")
  
  # Combine the waves
  result <- combine_waves_FINAL()
  
  # Save the combined dataset
  output_file <- "longitudinal_survey_data_FINAL_2002_2007.csv"
  write_csv(result$data, output_file)
  cat("\nSaved combined dataset:", output_file, "\n")
  
  # Save the comprehensive variable summary
  summary_file <- "variable_summary_FINAL_2002_2007.csv"
  write_csv(result$summary, summary_file)
  cat("Saved variable summary:", summary_file, "\n")
  
  # Save concept summary
  concept_file <- "concept_summary_FINAL_2002_2007.csv"
  ...
  log_data <- data.frame(
    ...
    immigration_attitude_vars = sum(result$summary$concept == "Immigration Attitudes" & result$summary$n_unique >= 2),
    demographic_vars = sum(result$summary$concept %in% c("Demographics", "Socioeconomic") & result$summary$n_unique >= 2),
    stringsAsFactors = FALSE
  )
  
  write_csv(log_data, "processing_log_FINAL_2002_2007.csv")
  cat("Saved processing log: processing_log_FINAL_2002_2007.csv\n")
  
  cat("\n=== FINAL DATASET SUMMARY ===\n")
  cat("📊 Total observations:", nrow(result$data), "\n")
  cat("📅 Years covered:", paste(sort(unique(result$data$survey_year)), collapse = ", "), "\n")
  cat("🏛️ Immigration attitude variables:", sum(result$summary$concept == "Immigration Attitudes" & result$summary$n_unique >= 2), "\n")
  cat("👥 Demographic variables:", sum(result$summary$concept %in% c("Demographics", "Socioeconomic") & result$summary$n_unique >= 2), "\n")
  cat("🌐 Immigration/Nativity variables:", sum(result$summary$concept == "Immigration/Nativity" & result$summary$n_unique >= 2), "\n")
  
  cat("\n=== Research Applications ===\n")
  cat("✅ How did Latino views on immigration levels change 2002-2007?\n")
  cat("✅ How did support for legalization vary by immigrant generation?\n") 
  cat("✅ What demographic factors predict immigration attitudes?\n")
  cat("✅ How did deportation concerns evolve over time?\n")
  ...
  cat("- ", summary_file, "\n") 
  cat("- ", concept_file, "\n")
  cat("- processing_log_FINAL_2002_2007.csv\n")
